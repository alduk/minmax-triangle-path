...
object MinMaxTrianglePath:
  def main(args: Array[String]): Unit =
    ...
    MinMaxTrianglePathLogic.solveTriangle(triangle, SearchType.Min) match
      case Some(result) =>
    ...
    MinMaxTrianglePathLogic.solveTriangle(triangle, SearchType.Max) match
      case Some(result) =>
        val pathStr = result.path.mkString(" + ")
